...
class MS_Deeplab(nn.Module):
    ...
    def forward(self,x):
        input_sizex = x.size()[2]
        input_sizey = x.size()[3]
        self.interp1 = nn.UpsamplingBilinear2d(size = (  int(input_sizex*0.75)+1,  int(input_sizey*0.75)+1  ))
        self.interp2 = nn.UpsamplingBilinear2d(size = (  int(input_sizex*0.5)+1,   int(input_sizey*0.5)+1   ))
        self.interp3 = nn.UpsamplingBilinear2d(size = (  outS(input_sizex),   outS(input_sizey)   ))
        out = []
        x2 = self.interp1(x)
        x3 = self.interp2(x)
