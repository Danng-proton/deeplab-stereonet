                       ...
                       int corr_type_multiply)
{
  ...
  // dxy:  one pixel with kernel_radius neighbor multi
  int kernel_radius = (kernel_size - 1) / 2;
  ////dxy  border_radius: cropped
  int border_radius = kernel_radius + max_displacement;

  // int paddedInputHeight = inputHeight + 2 * pad_size;
  int paddedInputHeight = inputHeight ;   //changed by dxy
  
  int paddedInputWidth = inputWidth + 2 * pad_size;
//dxy: max_displacement:disparity, but stride2=2, not pixel step corr.  at x,y axis
// max_displacement= 40 , stride2 can be changed to 1.  //changed by dxy
  // int nOutputChannels = ((max_displacement/stride2)*2 + 1) * ((max_displacement/stride2)*2 + 1);
  int nOutputChannels = ((max_displacement/stride2)*2 + 1) ;   //changed by dxy

  //dxy here output size to be changed.***********************************
  // int outputHeight = ceil((float)(paddedInputHeight - 2 * border_radius) / (float)stride1);
  int outputHeight = ceil((float)(paddedInputHeight-kernel_radius)/ (float)stride1);
                       ...
                       int corr_type_multiply)
{
  ...
  int paddedInputHeight = input1->size[2];//changed by dxy
