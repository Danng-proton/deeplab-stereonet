...
{
    // n (batch size), c (num of channels), y (height), x (width)  X,Y,C,N
    int n = blockIdx.x;
    int y = blockIdx.y;
    int x = blockIdx.z;

    int ch_off = threadIdx.x;     //by dxy: one block per pixel at N Y X, here is the CH_OFF chanel 
    float value;

    int dimcyx = channels * height * width;//by dxy: # of pixels per batch 
    int dimyx = height * width; //by dxy: # of pixel per channel

    int p_dimx = (width + 2 * pad_size); //by dxy:  target 
    // int p_dimy = (height + 2 * pad_size);
    int p_dimy = (height);//by dxy:  
    int p_dimyxc = channels * p_dimy * p_dimx;
    int p_dimxc = p_dimx * channels;   //by dxy: # per horizontal line

    for (int c = ch_off; c < channels; c += THREADS_PER_BLOCK) {
      // //by dxy:  N*C*Y*X
      value = input[n * dimcyx + c * dimyx + y * width + x]; 
      // rinput[n * p_dimyxc + (y + pad_size) * p_dimxc + (x + pad_size) * channels + c] = value;
      rinput[n * p_dimyxc + (y/*here no pad_size*/) * p_dimxc + (x + pad_size) * channels + c] = value;   //by dxy:rinput: N*Y*X*C
    }
}
                                     ...
                                     int stride2)
{
    ...
    int pInputHeight = inputHeight;//changed by dxy
    ...
    int n  = blockIdx.x;
    //by dxy: *stride1 should no use because tag_1
    // int y1 = blockIdx.y * stride1 + max_displacement + kernel_rad;    v2
    int y1 = blockIdx.y * stride1 + kernel_rad;  //   hight
    int x1 = blockIdx.z * stride1 + max_displacement + kernel_rad;
    int c = threadIdx.x;

    
    //changed by dxy: here get from rinput, so chanel first. N*Y*X*C
    int pdimyxc = pInputHeight * pInputWidth * nInputChannels;
    int pdimxc = pInputWidth * nInputChannels;
    int pdimc = nInputChannels;

    
    //changed by dxy:  N*C*Y*X  output
    int tdimcyx = nOutputChannels * outputHeight * outputWidth;
    int tdimyx = outputHeight * outputWidth;
    int tdimx = outputWidth;

    float nelems = kernel_size * kernel_size * pdimc;//by dxy: 1*1*inputC
    ...
    // tj=0 for horizontal only
    // for (int tj = -displacement_rad; tj <= displacement_rad; ++tj ) {
    for (int tj = 0;tj<1;tj++ ) {//by dxy:replace by  tj=0;
...
__global__ void Correlation_backward_input1(int item, float *gradInput1, int nInputChannels, int inputHeight, int inputWidth, 
  ...
  {
    // n (batch size), c (num of channels), y (height), x (width)
//by dxy: here why?????   v2:right see second dim3
    int n = item; 
    int y = blockIdx.x * stride1/* + pad_size*/; //by dxy: *stride1 should no use because tag_1
    int x = blockIdx.y * stride1 + pad_size;  // for  rinput index
    int c = blockIdx.z;
    //current H,W,C, 
    ...
    int ymin = (y - kernel_rad) / stride1;//by dxy: 
    ...
    int pInputWidth = inputWidth + 2 * pad_size;
    // int pInputHeight = inputHeight + 2 * pad_size;
    int pInputHeight = inputHeight ;//by dxy: 

    //by dxy: N*Y*X*C  rinput
    int pdimyxc = pInputHeight * pInputWidth * nInputChannels;
    int pdimxc = pInputWidth * nInputChannels;
    int pdimc = nInputChannels;
    //by dxy: N*C*Y*X  output
    int tdimcyx = nOutputChannels * outputHeight * outputWidth;
    int tdimyx = outputHeight * outputWidth;
    int tdimx = outputWidth;
    ...
    //by dxy: tch_off: this chanel is for (i2,j2), 
    for (int tc = tch_off; tc < nOutputChannels; tc += THREADS_PER_BLOCK) {

      int i2 = (tc % displacement_size - displacement_rad) * stride2;  //by dxy: x
      int j2 =0;// (tc / displacement_size - displacement_rad) * stride2;  //by dxy: y

      //by dxy: n,y+j2, x+i2,c
      int indx2 = n * pdimyxc + (y + j2)* pdimxc + (x + i2) * pdimc + c;//by dxy:  for rInput2
      ...
      for (int j = ymin; j <= ymax; ++j) {
        for (int i = xmin; i <= xmax; ++i) {
          ...
          prod_sum[tch_off] += gradOutput[tindx] * val2;   //by dxy: N*C*Y*X
        }
      }
    }
    __syncthreads();
                                    ...
                                    /*THCState_getCurrentStream(state)*/cudaStream_t stream)
{
    ...
    // tag_1  by dxy
    dim3 blocks_grid(batchSize, inputHeight, inputWidth);   // only for channels_first
    // dim3 blocks_grid(inputHeight, inputWidth,nOutputChannels); //by dxy: HWC  v2: wrong
    dim3 threads_block(THREADS_PER_BLOCK);
