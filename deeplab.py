...
parser.add_argument('--save_path', type=str, default='results/train-7-10/',
                    help='the path of saving checkpoints and log when training')
parser.add_argument('--test_save_path', type=str, default='results/test-710/',
                    help='the path of saving checkpoints and log when testing')
# 'results/8Xmulti/checkpoint_512_sceneflow_only.pth'
parser.add_argument('--resume', type=str, default='results/train-7-10/', help='resume path')
...
def train(dataloader, model,save_path,optimizer, log, epoch=0):
    ...
    for batch_idx, (imgL, imgR, disp_L) in enumerate(dataloader):
        ...
        # print(scoremap.shape,disp_L.shape)
        ...
        if batch_idx % args.print_freq == 0:
            ...
            info_str = ['score loss = {:.2f}({:.2f})'.format(loss_scoremap[0].val, loss_scoremap[0].avg)]
            ...
            scoremap_pred = F.interpolate(
                    ...
                    mode='bilinear',
                    align_corners=False)
            all_results = torch.zeros((2, 1, H, W))
            print(torch.max(scoremap_pred))
            all_results[-2, 0, :, :] = scoremap_pred[:, :]*13/255.0
            print(torch.max(disp_L))
